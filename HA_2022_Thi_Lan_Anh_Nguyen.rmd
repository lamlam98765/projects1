...
Part I: function visualization and optimization (using steepest ascent algorithm)

Part II: select a best subset of regressors to explain a sale price for houses with the help of a linear model. 
In short, the data provide 60 indicators and price of houses, we will try to fit the data into linear models and find the best fit (by using changes in 1-neighborhood method and Bayesian Information Criterion (BIC)).

Part III: Estimate matters of interest (meand and variance) by Monte Carlo simulation and importance sampling.
...
a) Download the dataset "houseprice.RData" and open it in R. Set up 30 iterations and 8 random starts for a random starts local search procedure.
